...
;;
;; lyqi-editing-commands.el - editing and tranformation commands
;;
...
(defun lyqi-change-alteration (direction)
  (let* ((syntax lp--current-syntax)
         (note (lyqi-find-note-backward syntax (point) t)))
    (when note
      (with-slots (pitch alteration) note
        (let ((new-alteration (cond ((> direction 0) ;; increment
...
(defun lyqi-change-octave (direction)
  (let* ((syntax lp--current-syntax)
         (note (lyqi-find-note-backward syntax (point) t)))
    (when note
      (with-slots (octave-modifier) note
		  (let ((new-octave (if (= 0 direction)
					0
				      (+ direction octave-modifier))))
  ...
  (let* ((limits (lyqi--block-limits))
    ...
    (save-excursion
      ...
      (loop with syntax = lp--current-syntax
            ...
            while (and form (>= (lp--marker form) beginning))
            if (lyqi-note-lexeme-p form)
            do (lyqi--note-to-relative form ref-note)
            if (lyqi-simple-note-form-p form)
            do (loop for lexeme in (slot-value form 'children)
                     if (lyqi-note-lexeme-p lexeme)
                     do ((lyqi--note-to-relative lexeme ref-note)
			 (setq ref-note lexeme)))))))
